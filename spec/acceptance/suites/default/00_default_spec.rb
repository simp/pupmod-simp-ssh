  ...
  hosts_as('server').each do |_server|
    ...
    context "on #{os}:" do
      ...
      context 'logging into machines as root' do
        ...
        it 'is able to ssh into localhost' do
          hosts.each do |host|
        ...
        it "is able to ssh into #{os}-client" do
      ...
      context 'with a test user' do
        ...
        it 'does not accept old ciphers when not enabled' do
        ...
        it 'prompts user to change password if expired and logging in with cert' do
        ...
        it 'prompts user to change password if expired' do
          # reset and expire testuser password
          on(hosts, 'echo password | passwd testuser --stdin')
      ...
      context 'with customized settings' do
        ...
        it 'coexists with additional settings via the sshd_config type' do
        ...
        it 'coexists with additional settings via the ssh_config type' do
          # Ensure the client is using the default test setup
        ...
        it 'is able to disable AuthorizedKeysFile management' do
          ...
          expect(file_content_on(server, '/etc/ssh/sshd_config').strip).not_to match('AuthorizedKeysFile\s*/foo/bar')
        end
      end

      context 'with default parameters' do
        it 'moves the /etc/localtime file temporarily if it exists' do
          on(client, 'if [ -f /etc/localtime ]; then mv /etc/localtime /etc/localtime.bak; fi')
        end
        it 'configures server with no errors when missing /etc/localtime' do
          enable_epel_on(server)
          enable_epel_on(client)
          set_hieradata_on(server, server_hieradata)
          apply_manifest_on(server, server_manifest, expect_failures: false)
        end
        it 'moves the /etc/localtime file back if it was moved' do
