...
describe 'ssh::server::conf' do
  ...
  context 'with default parameters' do
    context 'latest openssh_version and both simp_options::fips and fips_enabled false' do
      ...
      it { is_expected.to_not contain_sshd_config('ListenAddress') }
      ...
      it { is_expected.to_not contain_class('oath') }
      it { is_expected.to_not contain_file('/etc/pam.d/sshd') }
  ...
  context 'with latest ssh_version and custom parameters' do
   ...
   # appear in the manifests.
    ...
    context 'with pki enabled' do
      ...
      it { is_expected.to compile.with_all_deps }
      it { is_expected.to contain_pki__copy('sshd').with(
    ...
    context 'with manage_pam_sshd=true' do
      context 'with defaults' do
        ...
        it { is_expected.to compile.with_all_deps }
        it { is_expected.to contain_file('/etc/pam.d/sshd').with_content( <<~EOM
            #%PAM-1.0
            auth       required     pam_sepermit.so
            auth       substack     password-auth
            auth       include      postlogin
